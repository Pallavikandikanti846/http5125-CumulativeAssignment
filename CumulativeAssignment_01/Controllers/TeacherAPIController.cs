...
namespace CumulativeAssignment_01.Controllers
{
    ...
    public class TeacherAPIController : ControllerBase
    {
        ...
        public int addTeacher([FromBody]Teacher newTeacher)
        {
            ...
            //Checking when hiredate is greater than current date and returns -1 when condition is true
        ...
        /// <summary>
        /// This method is used to update a existing teacher details of eachers table. and returns teacher object if the teacher data is valid according to the condition, if not returns an error when the teacher first name or last name is empty or null, hire date is future date, salary is less than 0, and teacher id does not exist.
        /// </summary>
        /// <param name="teacherId">It takes the teacher id as an input from the user.</param>
        /// <returns>
        ///  The method returns the an updated teacher details from teachers table.
        /// </returns>
        /// <example>
        /// PUT: "https://localhost:7074/api/TeacherAPI/updateTeacher/10" 
        /// Header : Content-Type:"application/json"
        /// FORM DATA :
        /// {\"teacherfname\":\"John\",\"teacherlname\":\"Abhraham\",\"EmployeeNo\":\"T510\",\"salary\":\"55.67\"}
        ///{"teacherId":10,"teacherFName":"John","teacherLName":"Abhraham","employeeNo":"T510","hireDate":"0001-01-01T00:00:00","salary":55.67}
        ///PUT: "https://localhost:7074/api/TeacherAPI/updateTeacher/5" 
        /// FORM DATA :
        /// {\"teacherfname\":\"\",\"teacherlname\":\"Abhraham\",\"EmployeeNo\":\"T510\",\"salary\":\"55.67\"}
        /// Error:Teacher First Name and Last Name cannot be empty or null
        /// FORM DATA :
        /// "{\"teacherfname\":\"John\",\"teacherlname\":\"Abhraham\",\"EmployeeNo\":\"T510\",\"salary\":\"-10.00\"}
        /// Error:Teacher Salary Date cannot be less than 0
        /// PUT: "https://localhost:7074/api/TeacherAPI/updateTeacher/24" 
        /// {\"teacherfname\":\"Melissa\",\"teacherlname\":\"Mercer\",\"EmployeeNo\":\"T545\",\"salary\":\"55.67\"}
        /// Teacher Id 24 Not Found
        /// PUT: "https://localhost:7074/api/TeacherAPI/updateTeacher/8" 
        /// {\"teacherfname\":\"Momna\",\"teacherlname\":\"Sheikh\",\"EmployeeNo\":\"T543\",\"hiredate\":\"2025-04-24\",\"salary\":\"55.67\"}
        /// Error:Teacher Hire Date cannot be future date
        /// </example>
        ...
        {
            // Checking when teacher first name and last name is null or empty and returns the teacher data when condition is true
            if (string.IsNullOrWhiteSpace(teacherData.TeacherFName) || string.IsNullOrWhiteSpace(teacherData.TeacherLName))
            {
                Debug.WriteLine("Teacher First Name and Last Name cannot be empty or null");
                return BadRequest("Error:Teacher First Name and Last Name cannot be empty or null");
            }
            //Checking when hiredate is greater than current date and returns the teacher data when condition is true
            if (teacherData.HireDate > DateTime.Now)
            {
                Debug.WriteLine("Teacher Hire Date cannot be future date");
                return BadRequest("Error:Teacher Hire Date cannot be future date");
            }
            //Checking when salary is less than 0 and returns the teacher data when condition is true
            if (teacherData.Salary < 0)
            {
                Debug.WriteLine("Teacher Salary Date cannot be less than 0");
                return BadRequest("Error:Teacher Salary Date cannot be less than 0");
            }
            ...
            try
            {
                ...
                //writing the queries to update the desired data
                ...
                command.Parameters.AddWithValue("@employeenumber", teacherData.EmployeeNo);
                command.Parameters.AddWithValue("@hiredate", teacherData.HireDate);
                command.Parameters.AddWithValue("@salary", teacherData.Salary);
                command.Parameters.AddWithValue("@teacherid", teacherId);


                Debug.WriteLine("EmployeeNo: " + teacherData.EmployeeNo);
                Debug.WriteLine("FName: " + teacherData.TeacherFName);
                Debug.WriteLine("LName: " + teacherData.TeacherLName);
                Debug.WriteLine("HireDate: " + teacherData.HireDate);
                Debug.WriteLine("Salary: " + teacherData.Salary);
                ...
                //Attempting in C3 not done in C1 and C2
                //Checking condition for teacher id does not exist and returns error message not found.
                if (rowsReturned == 0)
                {
                    return NotFound("Teacher Id " + teacherId + " Not Found");
                }
            ...
            return Ok(GetATeacher(teacherId));
        }
    }
}
